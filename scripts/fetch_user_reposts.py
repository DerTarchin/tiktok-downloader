#!/usr/bin/env python3
"""Script to fetch TikTok user reposts using their web API."""
...
from downloader.tiktok_api import fetch_user_reposts, format_video_url, get_user_info

def main():
    parser = argparse.ArgumentParser(description='Fetch TikTok user reposts using web API')
    parser.add_argument('username', help='Username of the TikTok user (without @)')
    parser.add_argument('--output-file', '-o', help='Output file to write URLs to. If not specified, will use reposts_[username].txt')
    ...
    try:
        ...
        output_file = args.output_file or os.path.join(os.getcwd(), f'reposts_{args.username}.txt')
